...
namespace ClientUtils.RenderingPrimitives
{
...
public class MaterialLoader : IDisposable
{
    ...
    public MaterialLoader(MaterialResources MaterialResources)
    {
        materialResources = MaterialResources;
        _rumtime_map = new();
        _parameter_id_maps = new();

        FieldInfo[] fields = materialResources.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance);
        foreach (FieldInfo field in fields)
        {
            if (field.FieldType == typeof(UnityEngine.Material))
            {
                //add material to _runtime_map, indexed by the variable name
                var mat = field.GetValue(materialResources) as UnityEngine.Material;
                if (mat == null)
                {
                    Debug.Log("material field is null. Check ScriptableObject DefaultResources.");
                    continue;
                }
